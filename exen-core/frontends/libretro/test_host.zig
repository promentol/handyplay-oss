//! Minimal libretro host for the exen core: dlopen, drive the lifecycle, verify
//! retro_run renders + serialize/unserialize roundtrips. Run from the package dir
//! (reference/simulator.ini, assets/, flash/ must resolve). Usage:
//!   test_host <core.dylib> <game.exn>
const std = @import("std");

const MAXPX = 320 * 320;
var fb: [MAXPX]u32 = undefined;
var fb_w: c_uint = 0;
var fb_h: c_uint = 0;
...
fn videoCb(data: ?*const anyopaque, w: c_uint, h: c_uint, pitch: usize) callconv(.c) void {
    ...
    if (data) |d| {
        ...
        var y: usize = 0;
        while (y < h) : (y += 1) {
            @memcpy(std.mem.sliceAsBytes(fb[y * w ..][0..w]), src[y * pitch ..][0 .. w * 4]);
        }
    }
}
...
pub fn main() !void {
    ...
    i = 0;
    while (i < 60) : (i += 1) run();
    const fb1: [MAXPX]u32 = fb;
    var nz: usize = 0;
    for (fb1[0 .. fb_w * fb_h]) |p| {
        if (p & 0xFFFFFF != 0) nz += 1;
    }

    if (!unser(state.ptr, sz)) return error.UnserFailed;
    i = 0;
    while (i < 60) : (i += 1) run();
    const fb2: [MAXPX]u32 = fb;

    const match = std.mem.eql(u32, fb1[0 .. fb_w * fb_h], fb2[0 .. fb_w * fb_h]);
    std.debug.print("[host] fb {d}x{d} non-black={d}; serialize={d}B\n", .{ fb_w, fb_h, nz, sz });
