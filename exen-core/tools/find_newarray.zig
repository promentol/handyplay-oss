//! Scan all classes for NEWARRAY (0xbc) opcodes with given tag operand.
//! Reports class/method/PC for each hit.
//!
//! Usage:
//!   zig run tools/find_newarray.zig -- samples/Pikubi.exn 0x0099
...
pub fn main() !void {
    ...
    const tag_str = args.next() orelse return;
    const target_tag: u16 = try std.fmt.parseInt(u16, tag_str[2..], 16);
    ...
    std.debug.print("scanning for NEWARRAY tag=0x{x:0>4}\n\n", .{target_tag});
            ...
            while (pc < end and pc < cb.len) {
                ...
                switch (operandKind(op)) {
                    ...
                    .u16op => {
                        ...
                        if (op == 0xbc and v == target_tag) {
                            std.debug.print("  HIT class=0x{x:0>8} method=0x{x:0>8} PC=0x{x:0>4} tag=0x{x:0>4}\n",
                                .{ class_hash, mhash, opc_pc, v });
                            hits += 1;
                        }
                        pc += 2;
                        // For obj-array (tag 0x99 low byte), consume 1 extra u16
                        if (op == 0xbc and (v & 0xFF) == 0x99) {
                            pc = alignPc(pc);
                            if (pc + 2 <= end) pc += 2;
                        }
                    },
